namespace Fase3.src.adts
{
    public class Blockchain
    {
        ...
        public bool SearchByEmailAndPass(string email, string password)
        {
            ...
            while (current != null)
            {
                if (current.Data.Correo == email && current.Data.Contrasenia == password)
                {
                    return true;
                }
...
");
            ...
            while (current != null)
            {
                ...
                string userDataHtml = $"ID: {current.Data.ID}<br/>" +
                                      $"Nombre: {System.Security.SecurityElement.Escape(current.Data.Nombre)}<br/>" +
                                      $"Correo: {System.Security.SecurityElement.Escape(current.Data.Correo)}<br/>" +
                                      $"Rol: {System.Security.SecurityElement.Escape(current.Data.Rol)}";
    ...
    >];{7}",
                ...
                if (current.previous != null)
                {
                    ...
                    connectionsBuilder.AppendFormat(
                        "    block{0} -> block{1} [tailport=e, headport=w, color=\"#5e35b1\"];{2}",
                    ...
                    connectionsBuilder.AppendFormat(
                        "    block{1} -> block{0} [tailport=w, headport=e, color=\"#7e57c2\", style=dashed];{2}",
                        current.previous.Index, // {0}
                        current.Index,          // {1}
                        Environment.NewLine     // {2}
                    );
